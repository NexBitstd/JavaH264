...
mod internal {
    ...
    fn raw_fcc_to_frameformat(raw: OSType) -> Option<FrameFormat> {
        match raw {
            ...
            // OBS and other virtual cameras expose only 32-bit BGRA. AVFoundation converts it to YUYV
            // on the video-data output (see set_frame_format), so map it through YUYV instead of
            // rejecting the device with "Unknown FourCharCode".
            kCVPixelFormatType_32BGRA => Some(FrameFormat::YUYV),
            _ => None,
        }
    }
