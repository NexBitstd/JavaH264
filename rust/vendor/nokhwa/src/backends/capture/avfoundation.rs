...
impl CaptureBackendTrait for AVFoundationCaptureDevice {
    ...
    fn compatible_list_by_resolution(
        ...
        // Upstream bug: this filtered `!= fourcc`, dropping exactly the formats with the requested
        // FourCC, so a camera with a single FourCC (e.g. macOS built-in cams, all 420v/YUYV) reported
        // zero compatible formats. Keep the formats that match the requested FourCC.
